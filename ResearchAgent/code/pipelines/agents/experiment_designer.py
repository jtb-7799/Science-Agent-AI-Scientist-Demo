...
class ExperimentDesigner(BaseAgent):
    ...
    def _build_refinement_prompt(self, context) -> str:
        ...
        # Final
        prompt += (
            f"Finally, with these reviews and feedback about {list_of_items_to_grammatical_text(target_feedbacks)} in mind while maintaining the strengths in {list_of_items_to_grammatical_text(other_feedbacks)}, please craft a revised version of your experiment with its rationale.\n\nCRITICAL OUTPUT FORMAT — You MUST output EXACTLY in this format with these exact labels on their own lines:\nExperiment: <your revised experiment here>\nRationale: <your rationale here>\nDo NOT use markdown headers (###, ##), do NOT use bold labels (**Experiment**), and do NOT add any other sections."
        )
        return prompt

    def parse_output(self, text: str) -> Dict[str, str]:
        # Match "Experiment:" ... "Rationale:" with optional markdown (###, **, etc.)
        match = re.search(r"(?:#*\s*)?(?:\*\*)?Experiment(?:\*\*)?\s*:?\s*(.*?)[\s#]*(?:\*\*)?Rationale(?:\*\*)?\s*:?\s*(.*)", text, re.DOTALL | re.IGNORECASE)
        if match:
            return {'experiment': match.group(1).strip(), 'experiment_rationale': match.group(2).strip()}
        return {'experiment': text.strip(), 'experiment_rationale': ''}
