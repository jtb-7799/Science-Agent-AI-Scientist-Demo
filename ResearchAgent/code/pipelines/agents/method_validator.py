...
class MethodValidator(BaseAgent):
    ...
    def _build_validation_prompt_for_clarity(self, context: Dict) -> str:
        ...
        prompt += (
            ...
            "After your evaluation, please provide your review, feedback, and rating.\n\nCRITICAL OUTPUT FORMAT — You MUST output EXACTLY in this format:\nReview: <your detailed review here>\nFeedback: <your constructive feedback here>\nRating (1-5): <a single digit 1-5>\nDo NOT use markdown headers (###, ##), do NOT use bold labels, and do NOT add any other sections."
        )
        return prompt
    ...
    def parse_output(self, text: str) -> Dict[str, Any]:
        # Flexible matching with optional markdown and line breaks
        match = re.search(r"(?:#*\s*)?(?:\*\*)?Review(?:\*\*)?\s*:?\s*(.*?)[\s#]*(?:\*\*)?Feedback(?:\*\*)?\s*:?\s*(.*?)[\s#]*(?:\*\*)?Rating(?:\*\*)?(?:\s*\(1-5\))?\s*:?\s*([1-5])", text, re.DOTALL | re.IGNORECASE)
        if match:
            return {'review': match.group(1).strip(), 'feedback': match.group(2).strip(), 'rating': int(match.group(3))}
        return {'review': '', 'feedback': '', 'rating': 0}
