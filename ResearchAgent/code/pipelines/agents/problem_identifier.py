...
class ProblemIdentifier(BaseAgent):
    ...
    def _build_generation_prompt(self, context) -> str:
        ...
        prompt += (
            ...
            "Then, following your review of the above content, please proceed to generate one research problem with the rationale.\n\nCRITICAL OUTPUT FORMAT — You MUST output EXACTLY in this format with these exact labels on their own lines:\nProblem: <your problem statement here>\nRationale: <your rationale here>\nDo NOT use markdown headers (###, ##), do NOT use bold labels (**Problem**), and do NOT add any other sections."
        )
        return prompt
    ...
    def _build_refinement_prompt(self, context) -> str:
        ...
        # Final
        prompt += (
            f"Finally, with these reviews and feedback about {list_of_items_to_grammatical_text(target_feedbacks)} in mind while maintaining the strengths in {list_of_items_to_grammatical_text(other_feedbacks)}, please craft a revised version of the problem with the rationale.\n\nCRITICAL OUTPUT FORMAT — You MUST output EXACTLY in this format with these exact labels on their own lines:\nProblem: <your revised problem here>\nRationale: <your rationale here>\nDo NOT use markdown headers (###, ##), do NOT use bold labels (**Problem**), and do NOT add any other sections."
        )
        return prompt

    def parse_output(self, text: str) -> Dict[str, str]:
        # Match "Problem:" ... "Rationale:" with optional markdown (###, **, etc.)
        match = re.search(r"(?:#*\s*)?(?:\*\*)?Problem(?:\*\*)?\s*:?\s*(.*?)[\s#]*(?:\*\*)?Rationale(?:\*\*)?\s*:?\s*(.*)", text, re.DOTALL | re.IGNORECASE)
        if match:
            return {'problem': match.group(1).strip(), 'problem_rationale': match.group(2).strip()}
        return {'problem': text.strip(), 'problem_rationale': ''}
