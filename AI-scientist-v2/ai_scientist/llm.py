...
def _resolve_model(model):
    """Resolve model name for Azure deployment. Returns the Azure deployment name if mapped, otherwise the original model."""
    if _use_azure():
...
def _create_openai_client(**kwargs):
    """Create an OpenAI client, using Azure if configured."""
    if _use_azure():
        return AzureOpenAI(
...
def create_client(model) -> tuple[Any, str]:
    if model.startswith("claude-"):
        ...
        if _use_azure():
            print(f"Using Azure OpenAI API with model {model}.")
        else:
