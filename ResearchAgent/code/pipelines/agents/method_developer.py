...
class MethodDeveloper(BaseAgent):
    ...
    def _build_generation_prompt(self, context) -> str:
        ...
        prompt += (
            ...
            f"Research problem: {context.get('problem')} \nRationale: {context.get('problem_rationale')} \n\n"
            "Then, following your review of the above content, please proceed to propose your method with its rationale.\n\n"
            "CRITICAL OUTPUT FORMAT — You MUST output EXACTLY in this format with these exact labels on their own lines:\n"
            "Method: <your proposed method here>\n"
            "Rationale: <your rationale here>\n"
            "Do NOT use markdown headers (###, ##), do NOT use bold labels (**Method**), and do NOT add any other sections."
        )
        return prompt
    ...
    def _build_refinement_prompt(self, context) -> str:
        ...
        # Final
        prompt += (
            f"Finally, with these reviews and feedback about {list_of_items_to_grammatical_text(target_feedbacks)} in mind while maintaining the strengths in {list_of_items_to_grammatical_text(other_feedbacks)}, please craft a revised version of the method with the rationale.\n\n"
            "CRITICAL OUTPUT FORMAT — You MUST output EXACTLY in this format with these exact labels on their own lines:\n"
            "Method: <your revised method here>\n"
            "Rationale: <your rationale here>\n"
            "Do NOT use markdown headers (###, ##), do NOT use bold labels (**Method**), and do NOT add any other sections."
        )
        return prompt
    ...
    def parse_output(self, text: str) -> Dict[str, str]:
        # Remove debug logging now that we know the issue
        match = re.search(r"(?:#*\s*)?(?:\*\*)?Method(?:\*\*)?\s*:?\s*(.*?)[\s#]*(?:\*\*)?Rationale(?:\*\*)?\s*:?\s*(.*)", text, re.DOTALL | re.IGNORECASE)
        if match and match.group(2).strip():
            return {'method': match.group(1).strip(), 'method_rationale': match.group(2).strip()}
        return {'method': text.strip(), 'method_rationale': ''}
