...
def compile_latex(cwd, pdf_file, timeout=30):
    ...
    # Auto-fix: insert \begin{document} if LLM omitted it
    tex_path = osp.join(cwd, "template.tex")
    if os.path.exists(tex_path):
        with open(tex_path, "r") as f:
            content = f.read()
        if "\\begin{document}" not in content and "\\maketitle" in content:
            content = content.replace("\\maketitle", "\\begin{document}\n\\maketitle")
            with open(tex_path, "w") as f:
                f.write(content)
            print("Auto-inserted missing \\begin{document}")
