...
def overall_summarize(journals, cfg=None):
    ...
    with ThreadPoolExecutor() as executor:
        ...
        # Group summaries by main stage number (handles sub-stages)
        stage_map = {}
        for i, (stage_name, _) in enumerate(journals):
            main_stage = stage_name.split("_")[0] if "_" in stage_name else stage_name
            if main_stage not in stage_map:
                stage_map[main_stage] = results[i]

        # Extract in order: stage 1=draft, 2=baseline, 3=research, 4=ablation
        draft_summary = stage_map.get("1", {})
        baseline_summary = stage_map.get("2", {})
        research_summary = stage_map.get("3", {})
        ablation_summary = stage_map.get("4", {})
